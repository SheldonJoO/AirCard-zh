...
// MARK: - Localization Helper (中文版)
/// 将英文原文作为键，从 zh-Hans.lproj/Localizable.strings 查表返回中文。
/// App 的 Info.plist 已将 CFBundleDevelopmentRegion 设为 zh-Hans，因此界面默认显示中文。
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(alignment: .top, spacing: 6) {
                ...
                if vm.passcodeLanguageTarget == .all && vm.passcodeBoldTarget == .both {
                    ...
                    Text("Fast mode selected: only targets \(L(vm.passcodeLanguageTarget.rawValue)) with \(L(vm.passcodeBoldTarget.rawValue)).")
                        .font(.system(size: 9))
                        .foregroundColor(.primary)
                        .fixedSize(horizontal: false, vertical: true)
                }
