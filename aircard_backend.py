...
def cmd_flash(udid: str, card_hash: str, image_path: str) -> bool:
    ...
    print(json.dumps({
        ...
        "step": step,
        "total": total_steps,
        "message": f"正在写入 {len(asset_payloads)} 个卡片素材文件（快速批处理）…"
    }))
    sys.stdout.flush()
...
def cmd_flash_passthm(
    ...
    try:
        ...
        print(json.dumps({
            ...
            "step": 0,
            "total": total_steps,
            "message": f"正在刷写密码主题「{path.stem}」（共 {total_steps} 个素材）…"
        }))
        sys.stdout.flush()
        ...
        for tdir, dir_files in items_by_dir.items():
            ...
            print(json.dumps({
                ...
                "step": base_step,
                "total": total_steps,
                "message": f"正在将 {len(dir_files)} 个素材刷写到 {tdir_name}…"
            }))
            sys.stdout.flush()
            ...
            if not ok:
                ...
                print(json.dumps({
                    "type": "warning",
                    "message": f"批量写入 {tdir_name} 失败，回退为逐个文件写入…"
                }))
                sys.stdout.flush()
                ...
                for f_idx, (leaf, payload) in enumerate(dir_files, 1):
                    ...
                    print(json.dumps({
                        ...
                        "total": total_steps,
                        "leaf": leaf,
                        "message": f"[回退] 正在写入 {leaf}（{curr}/{total_steps}）…"
                    }))
                    sys.stdout.flush()
                ...
                if failed_leaves:
                    print(json.dumps({
                        "type": "error",
                        "message": f"无法写入 {tdir_name} 中的 {len(failed_leaves)} 个文件：{', '.join(failed_leaves[:5])}"
                    }))
                    sys.stdout.flush()
                    return False
        ...
        print(json.dumps({
            ...
            "step": total_steps,
            "total": total_steps,
            "message": f"密码主题「{path.stem}」已成功应用！锁定 iPhone 即可查看。"
        }))
        sys.stdout.flush()
        return True
