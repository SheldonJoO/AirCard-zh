...
def capture_card_hashes(udid: str, existing_cards: list[str] | None = None) -> list[str]:
    ...
    print("=" * 60)
    print("识别你的卡片：")
    print("  👉 1) 双击侧边（电源）按钮打开 Apple Pay。")
    print("  👉 2) 通过 Face ID 验证。")
    print("  👉 3) 轻点你的卡片以触发实时识别！")
    print("完成后按回车键。")
    print("=" * 60 + "\n")
...
def main():
    print("=" * 60)
    print("🎴 AirCard — Apple 钱包卡片换肤工具（基于 airlift）")
    print("=" * 60)
    ...
    if not device:
        print("❌ 未找到 iPhone！请用 USB 连接 iPhone 并解锁屏幕。")
        sys.exit(1)
    ...
    if not operation_ok(probe):
        print("❌ airlift 预检失败。请确保设备已配对并信任。")
        sys.exit(1)

    # 3. Card discovery / selection
    saved_cards = load_saved_cards()
    print(f"\n[2/5] 已保存卡片：{len(saved_cards)}")
